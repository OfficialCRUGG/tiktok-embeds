...
export function getMetaTags(data: tiktok.Result, protocol: string, hostname: string, vmId: string): string {
  ...
  const CONSTS = {
    ...
    url: `https://vm.crugg.de/${vmId}.mp4`,
  ...
  const metaTags = [
    ...
    `<link rel="alternate" href="https://vm.crugg.de/oembed.json?description=${encodeURIComponent(CONSTS.description)}&title=${encodeURIComponent(CONSTS.title)}&url=${encodeURIComponent(CONSTS.tikTokUrl)}&siteTitle=${encodeURIComponent(CONSTS.siteTitle)}&siteUrl=${encodeURIComponent(protocol + '://' + hostname)}" type="application/json+oembed" title="@CRUGG on TikTok">`,
  ];
